...
const nav = [
  { name: "Tableau de bord", to: "/", icon: FileText },
      ...
      <header className="flex items-center justify-between px-10 py-4 bg-white border-b border-muted shadow-sm">
        <div className="flex items-center gap-3">
          <img
            src="/placeholder.svg"
            alt="Logo crèche"
            className="w-10 h-10 rounded-full mr-3 border bg-muted"
          />
        ...
        <nav className="flex gap-2">
          {nav.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`flex items-center px-3 py-2 rounded-md text-sm font-medium gap-2 hover:bg-muted transition ${
                pathname === item.to ? "bg-accent font-semibold" : ""
              }`}
            >
