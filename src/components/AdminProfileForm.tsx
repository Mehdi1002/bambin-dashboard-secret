...
export default function AdminProfileForm() {
  ...
  return <>
      <form className="bg-white shadow rounded-lg max-w-2xl mx-auto p-8 space-y-5 border" onSubmit={handleSubmit}>
        <div className="flex items-center justify-center space-x-5">
          ...
          {form.logo && <img src={form.logo} alt="Logo" className="w-20 h-20 rounded-full border object-cover" />}
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs mb-1">Nom</label>
            <input name="nom" value={form.nom} onChange={handleChange} className="border rounded px-3 py-2 w-full" required />
          </div>
          <div>
      ...
      <div className="max-w-2xl mx-auto mt-8 bg-gray-50 rounded-lg p-6 border border-gray-200 shadow-sm">
        <div className="flex items-center gap-4 mb-4">
          {form.logo && <img src={form.logo} alt="Logo" className="w-16 h-16 rounded-full border object-cover" />}
          <div>
