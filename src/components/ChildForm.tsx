...
export default function ChildForm({ initial, onSubmit, onCancel }: Props) {
  ...
  return (
    <form
      className="bg-white rounded-lg shadow max-w-2xl mx-auto p-7 border"
    ...
    >
        ...
        <div>
          ...
          {form.photo && (
            <img
              src={form.photo}
              alt="Photo"
              className="w-14 h-14 mt-2 rounded-full object-cover border"
            />
          )}
        </div>
      </div>
      <div className="flex gap-4 justify-end">
        <button
          type="button"
          onClick={onCancel}
          className="bg-muted text-primary border border-primary px-6 py-2 rounded hover:bg-muted/80"
        >
          Annuler
        </button>
        <button
